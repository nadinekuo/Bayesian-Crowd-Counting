...
def find_dis(point):   # NOTE: this is a (NUM_POINTS, 3)-shaped arr
    print(len(point))
    square = np.sum(point*points, axis=1)
    dis = np.sqrt(np.maximum(square[:, None] - 2*np.matmul(point, point.T) + square[None, :], 0.0))
    # For imgs with less than 3 points, just use max neighbours available
    num_neighs = 3 if len(point) >= 3 else len(point)
    dis = np.mean(np.partition(dis, num_neighs-1, axis=1)[:, 0:num_neighs+1], axis=1, keepdims=True)
    # print(dis.shape)
    return dis   # (NUM_POINTS, 1) shaped i.e. for all points the avg dist to 3 nearest neighbours
    ...
    for phase in ['Train', 'Test']:
        ...
        if phase == 'Train':
            ...
            for sub_phase in sub_phase_list:
                ...
                with open('IOCfish/{}.txt'.format(sub_phase)) as f:   # train.txt or val.txt containing rows of img_XXXX.jpg
                    for i in f:
                        ...
                        if sub_phase == 'train':  # NOTE: For train set, we also append the avg distance to nearest 3 ann points
                            dis = find_dis(points)
